...
def test_update_email(authorized_client, mock_mongo_client):
    ...
    # Mock find() for email check to return None (email available)
    # The find() iterates collections.
    # We need detailed mock control here.
    # The code: if await UserCRUD(users_db).find(username=user_update.email): raise 409
    
    # We mock find_one to return None first (email check), then return user (auth check)
