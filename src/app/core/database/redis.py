    ...
    try:
      ...
      if not alive:
        logger.error(
          {
            "message": (
              f"Couldn't connect to Redis: {settings.REDIS_HOST}:{settings.REDIS_PORT}."
            )
          },
          exc_info=True,
        )
      ...
      logger.error(
        {
          "message": "An error occured while connecting to Redis.",
          "detail": str(e),
        }
      )
  ...
  async def close(cls):
    ...
    if cls._client is not None:
      try:
        ...
        logger.error(
          {
            "message": "[x] Error closing Redis connection.",
            "detail": str(e),
          },
          exc_info=True,
        )
